...
- 教育检索实验闭环：每个教育 Run 冻结检索策略、知识依赖图和（如使用 `CALIBRATED` / `ADAPTIVE`）权重或策略选择快照；实验摘要支持按策略比较证据覆盖、前置缺口覆盖、冗余、证据效用/千字符、目标达成和掌握度变化，教师评价可追溯到真实 citation，形成性结果按学习状态分层参与后续校准；`GET /api/education/retrieval-policy` 返回当前学习者最近一次教育状态下的推荐策略、选择理由和候选结果
