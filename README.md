...
教育检索默认以向量与关键词融合召回候选证据，再在课程硬约束内结合目标知识点、前置知识缺口、知识依赖图覆盖、学习者掌握度和难度适配进行证据集合选择。当前选择器把缺口的 deficit、状态不确定性、保持度风险与依赖深度共同转化为可解释的边际收益，在目标锚点之后优先补齐更靠近目标的依赖前沿，并在上下文预算内施加冗余惩罚。`FULL`、`FULL_POINT_ESTIMATE`、`VECTOR_ONLY`、`KEYWORD_ONLY`、`NO_LEARNER_STATE`、`NO_DEPENDENCY_GRAPH` 和 `STATIC_WEIGHT` 用于可复现基线与消融；其中 `FULL_POINT_ESTIMATE` 保留保持度校正、知识依赖图和自适应权重，但关闭 Wilson 置信下界与区间宽度信号，用于单独识别状态不确定性的贡献。`CALIBRATED` 会读取当前租户已有的教师证据标注，并按 Run 创建时冻结的低掌握度、平衡掌握度或高掌握度状态分层收缩校准目标 grounding、前置补强、难度适配和总体效用权重。`ADAPTIVE` 会根据当前 Run 冻结的学习状态，在 `FULL`、`CALIBRATED`、`NO_DEPENDENCY_GRAPH` 和 `STATIC_WEIGHT` 候选中读取历史形成性测评结果，使用带中性先验收缩的掌握度增益、正确率和目标达成率综合分数选择下一策略；`BALANCED_EXPERIMENT` 则按同一状态条件下各候选方法的历史分配次数选择最少者，用于减少实验样本不均衡和手动选择偏差。两种分配器都会把 requested strategy、effective strategy、状态条件、候选统计和（如适用）校准快照冻结到 Run 中。状态分层样本不足时回退租户级权重，历史 Run 不会因后续标注、测评或分配结果变化而漂移。实验聚合器按实际执行方法统计结果，并在同一学习者-学习目标内将各方法与 `FULL` 配对，计算掌握度增益、目标达成率、达标轮次和前置缺口覆盖的差值；另提供请求策略→实际策略的分配审计，避免把分配器误当成检索方法或把学习者个体差异误当成检索收益。

面向编程学习时，课程资料可额外绑定 `programmingLanguage`（如 `PYTHON`、`JAVA`）。教育 Run 会在创建时冻结同一语言上下文，语言标签作为可回放的硬检索约束；知识依赖图同时按语言范围保存边：请求指定语言时合并通用边与同语言边，排除其他语言边；未填写语言的历史资料和历史依赖边继续作为通用课程资料兼容使用。代码评测诊断还会归一为 `SYNTAX`、`STRUCTURE`、`IDENTIFIER`、`TYPE`、`DEPENDENCY`、`COMPILATION`、`TIMEOUT` 或 `UNKNOWN`，用于跨语言统计和选择下一步诊断练习；该标签是形成性证据，不是自动教师评分，也不会单独推出知识点掌握结论。

为保证状态 × 知识依赖图联合消融的可解释性，`NO_STATE_NO_GRAPH` 保留固定的向量+关键词混合召回，但同时关闭学习者状态和知识依赖图；它是主 2×2 消融的无状态无图控制组，`VECTOR_ONLY` 仅作为独立召回模态基线。

知识源元数据保存时会把同一课程版本下的依赖边合并校验为 DAG；如果新资料与已有资料形成循环依赖，系统返回 `EDUCATION_DEPENDENCY_CYCLE` 并回滚本次元数据事务，不生成不可解释的图快照。

代码评测形成的诊断类别会回流到下一步学习推荐：语法/结构、标识符、类型、依赖、编译和超时问题分别生成针对性的解释、最小修改和独立测试任务；系统不会把“编译通过”直接当作概念掌握。教师还可以为每个行为测试用例标注具体知识点；提交后的加权通过率会分别回写目标知识点和标注的前置知识，未标注用例回退到作业目标，并在形成性测评的结构化证据中保留用例到知识点的映射。
...
- 教育检索实验闭环：每个教育 Run 冻结检索策略、知识依赖图和（如使用 `CALIBRATED` / `ADAPTIVE` / `BALANCED_EXPERIMENT`）权重或策略选择快照；实验摘要支持按实际执行方法比较证据覆盖、前置缺口覆盖、冗余、证据效用/千字符、目标达成和掌握度变化，教师评价可追溯到真实 citation，形成性结果按学习状态分层参与后续校准；同时保留请求策略→实际策略和状态条件的分配审计，`GET /api/education/retrieval-policy` 返回当前学习者最近一次教育状态下的推荐策略、选择理由和候选结果
- 论文实验离线复现：`scripts/education_experiment_analysis.py` 直接读取逐 Run 去标识样本，按线上口径输出策略聚合、状态/语言分层、FULL 配对和状态 × 知识依赖图四臂协同分析；脚本只使用 Python 标准库，统计门槛与结果解释见 [docs/education-experiment-analysis.md](docs/education-experiment-analysis.md)
...
- 本地演示启动后可按使用目的选择“管理员：系统设置”“老师：查看示例课程”“老师：从零开始开课”“学生：查看示例课程”或“学生：第一次使用”；其中从零开始的账号（`teacher-first-demo` / `student-first-demo`）没有历史资料、课程和作业，适合验证新用户首次进入系统的引导路径。右上角退出或切换身份后，工作台会重新加载。
...
- `POST/GET /api/education/goals`：创建和查询绑定学习者画像的结构化学习目标；目标测评、推荐和保持度计划接口位于 `/api/education/goals/{goalId}/...`，推荐会返回依赖图上优先补强的前置知识及缺口，目标不能通过状态接口直接改为 `COMPLETED`
...
- `POST /api/education/courses/{courseId}/assignments`：向课程活跃名单批量布置统一目标，支持可选编程语言上下文，必须携带 `Idempotency-Key`；同一课程和幂等键会复用原批次，同一键提交不同内容（包括不同语言）会返回 `409 ASSIGNMENT_BATCH_KEY_REUSED_WITH_DIFFERENT_REQUEST`
...
- `POST /api/education/assignments/{assignmentId}/start`：学习者接受（如尚未接受）并直接启动第一步教育 Run；返回绑定的学习会话，作业要求、编程语言和未完成的“补证据/重新学习”教师干预会冻结到 Run 上下文；失败、超时或取消的 Run 会从该入口进入下一轮重试，启动请求不能覆盖教师冻结的语言
