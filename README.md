...
聊天工作台使用会话接口将每轮用户消息和助手结果持久化到数据库。每条用户消息都会创建一个关联 Run；上下文未接近租户输入上限时保留最近的已完成消息，超过预算后把较早消息压缩到会话上下文快照中，同时保留完整原始消息。这样长会话不会因为历史累积直接超过 `maxInputLength`，Rabbit 异步模式下助手气泡先显示“Agent 执行中”，Worker 完成后自动回写最终内容。当前会话可在标题栏或 `⌘/Ctrl + K` 命令面板中重命名，修改只影响当前租户/用户可见的会话标题，不改变工作区绑定或既有 Run。
