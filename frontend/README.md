...
这是基于 Vue 3 和 Vite 的 Harness 聊天工作台，默认提供持久化会话、消息气泡、逐轮输入和 Agent 执行状态轮询；运行控制台仍覆盖 Run 操作、审批审计和上下文文档。
...
运行控制台的“上下文治理”面板包含知识文档导入区，可拖入或选择 PDF/DOCX。上传成功后 Runtime 会提取文本、创建 chunk 并提交向量索引；扫描型 PDF 需要先 OCR。浏览器聊天区的“附件”仍是工作区文本附件，两条上传链路用途不同。
